"""
Vriddhi Alpha Finder - Complete FastAPI Backend
AI-Powered Personal Investment Advisor with Modern Portfolio Theory
"""
...
def advanced_stock_selector(df: pd.DataFrame, expected_cagr: float, horizon_months: int):
    ...
    Round 1: Best stock per sector with lowest PEG ratio
    ...
    feasible = True  # Focus on best recommendations
...
def compute_projections(optimized_df: pd.DataFrame, monthly_investment: float,
    ...
    for month in timeline:
        # Sum of future values of all monthly investments
        total_value = 0
...
def calculate_whole_share_allocation(optimized_df: pd.DataFrame, monthly_investment: float):
    """
    Calculate whole share allocations and fractional handling
    """
    whole_share_data = []
    total_allocation = 0
    fractional_amount = 0
    ...
    for _, row in optimized_df.iterrows():
        allocation_amount = row['Weight'] * monthly_investment
        current_price = row['Current_Price']

        if current_price > 0:
            shares_possible = int(allocation_amount // current_price)
            amount_used = shares_possible * current_price
            fractional_leftover = allocation_amount - amount_used
        else:
            shares_possible = 0
            amount_used = 0
            fractional_leftover = allocation_amount

        whole_share_data.append({
            'ticker': row['Ticker'],
            'sector': row['Sector'],
            ...
            'fractional_leftover': fractional_leftover
        })

        total_allocation += amount_used
        fractional_amount += fractional_leftover

    return {
        'whole_share_data': whole_share_data,
        'total_allocated': total_allocation,
        'fractional_amount': fractional_amount,
        'efficiency': (total_allocation / monthly_investment) * 100
    }
...
async def startup_event():
    ...
    logger.info("🚀 Starting Vriddhi Alpha Finder API...")
    if load_stock_data():
...
async def root():
    ...
    return {
        "message": "Vriddhi Alpha Finder API v2.0",
        "description": "AI-Powered Personal Investment Advisor with Modern Portfolio Theory",
        "features": [
        ...
        "endpoints": {
            "health": "/health",
            "docs": "/docs",
            "investment_plan": "/api/v1/investment/plan"
        },
        "status": "operational"
    }
...
async def create_investment_plan(request: InvestmentRequest):
    ...
    try:
        ...
        logger.info(f"🔍 Selecting stocks for {request.investment_horizon_months} month horizon...")
        ...
        logger.info("⚖️ Optimizing portfolio weights using MPT...")
        ...
        logger.info("📊 Computing SIP growth projections...")
        projections = compute_projections(
            optimized_df, request.monthly_investment,
            request.investment_horizon_months, achieved_cagr
        )

        # Step 4: Whole Share Analysis
        whole_share_analysis = calculate_whole_share_allocation(optimized_df, request.monthly_investment)
        ...
        return InvestmentResponse(
            ...
            whole_share_analysis=whole_share_analysis
        )
...
@app.get("/api/v1/stocks/universe")
async def get_stock_universe():
    """Get information about available stocks"""
    if STOCK_DATA is None:
        raise HTTPException(status_code=500, detail="Stock database not loaded")

    return {
        "total_stocks": len(STOCK_DATA),
        "sectors": STOCK_DATA['Sector'].unique().tolist(),
        "sample_stocks": STOCK_DATA[['Ticker', 'Sector', 'Current_Price']].head(10).to_dict('records')
    }

if __name__ == "__main__":
    import uvicorn
    port = int(os.getenv("PORT", 8000))
    uvicorn.run("app.advanced_main:app", host="0.0.0.0", port=port, reload=True)